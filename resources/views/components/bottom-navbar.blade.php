<nav class="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-lg border border-gray-200 rounded-full shadow-xl px-3 py-2 flex items-center gap-2">
    @auth
        @if(auth()->user()->isAdmin())
            <a href="{{ route('admin.dashboard') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('admin.dashboard') ? 'bg-white text-gray-900 font-bold shadow-md border border-gray-200' : 'text-gray-500 hover:bg-gray-100' }}">
            ...
            <a href="{{ route('admin.applications.index') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('admin.applications.*') ? 'bg-white text-gray-900 font-bold shadow-md border border-gray-200' : 'text-gray-500 hover:bg-gray-100' }}">
                ...
                <span class="text-[10px] mt-0.5 font-medium">{{ __('Applications') }}</span>
            </a>
        ...
        <form method="POST" action="{{ route('logout') }}" class="flex flex-col items-center justify-center">
            @csrf
            <button type="submit" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition text-gray-500 hover:bg-gray-100 cursor-pointer">
                ...
                <span class="text-[10px] mt-0.5 font-medium">{{ __('Log Out') }}</span>
            </button>
        </form>
    @else
        <a href="{{ route('home') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('home') ? 'bg-white text-gray-900 font-bold shadow-md border border-gray-200' : 'text-gray-500 hover:bg-gray-100' }}">
