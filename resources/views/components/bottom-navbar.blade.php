<nav class="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-lg border border-gray-200 rounded-full shadow-xl px-3 py-2 flex items-center gap-2">
    @auth
        @if(auth()->user()->isRider())
            <a href="{{ route('dashboard') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('dashboard') ? 'bg-primary-500 text-white' : 'text-gray-500 hover:bg-gray-100' }}">
            ...
            <a href="{{ route('rides.create') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition text-gray-500 hover:bg-gray-100">
        ...
        @elseif(auth()->user()->isDriver())
            <a href="{{ route('driver.dashboard') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('driver.dashboard') ? 'bg-primary-500 text-white' : 'text-gray-500 hover:bg-gray-100' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-4 0a1 1 0 01-1-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 01-1 1h-2z"/></svg>
                <span class="text-[10px] mt-0.5 font-medium">{{ __('app.dashboard') }}</span>
            </a>
        @endif

        <a href="{{ route('home') }}#about" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition text-gray-500 hover:bg-gray-100">
        ...
        <a href="{{ route('profile.edit') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('profile.edit') ? 'bg-primary-500 text-white' : 'text-gray-500 hover:bg-gray-100' }}">
    ...
    @else
        <a href="{{ route('home') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('home') ? 'bg-primary-500 text-white' : 'text-gray-500 hover:bg-gray-100' }}">
        ...
        <a href="{{ route('login') }}" class="flex flex-col items-center justify-center px-4 py-2 rounded-full transition {{ request()->routeIs('login') ? 'bg-primary-500 text-white' : 'text-gray-500 hover:bg-gray-100' }}">
