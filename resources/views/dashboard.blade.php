                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8"
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden order-2 lg:order-1">
                        <div class="p-4 border-b border-gray-100">
                            <h3 class="text-sm font-semibold text-gray-700">{{ __('app.route_map') }}</h3>
                            <p class="text-xs text-gray-400 mt-1">{{ __('app.click_map_to_set_points') }}</p>
                        </div>
                        ...
                        <div class="p-4 border-t border-gray-100 space-y-2" x-show="distance > 0" x-transition>
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-gray-500">{{ __('app.route_distance') }}</span>
                    ...
                    <div class="bg-white rounded-2xl p-6 sm:p-8 border border-gray-100 order-1 lg:order-2">
                        ...
                        <form method="POST" action="{{ route('rides.store') }}" id="booking-form">
                            @csrf
                                    ...
                                    @foreach($services as $service)
                                        <label class="relative cursor-pointer"
                                            ...
                                            <div class="p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 text-center"
                                                ...
                                                <div class="text-2xl sm:text-3xl mb-1">{{ $service->icon }}</div>
                                                <p class="font-bold text-gray-900 text-sm">{{ $service->name }}</p>
                                                <p class="text-xs text-gray-500 mt-1">TK {{ number_format($service->base_fare, 0) }}</p>
                                            </div>
                                        </label>
                                    @endforeach
                                </div>
                            ...
                            <!-- Pickup Address -->
                            <div class="mb-3">
                                <label for="pickup_address" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('app.pickup_address') }}</label>
                                <div class="flex gap-2">
                                    <div class="flex-1 relative">
                                        <input type="text" name="pickup_address" id="pickup_address"
                                               x-model="pickupAddress"
                                               @input.debounce.500ms="geocodePickup()"
                                               class="input-field pr-8" placeholder="{{ __('app.pickup_placeholder') }}" required>
                                    ...
                                    <button type="button" @click="setPickupFromMap()"
                                        class="px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition" title="{{ __('app.set_from_map') }}">
                            ...
                            <!-- Dropoff Address -->
                            <div class="mb-4">
                                <label for="dropoff_address" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('app.dropoff_address') }}</label>
                                <div class="flex gap-2">
                                    <div class="flex-1 relative">
                                        <input type="text" name="dropoff_address" id="dropoff_address"
                                               x-model="dropoffAddress"
                                               @input.debounce.500ms="geocodeDropoff()"
                                               class="input-field pr-8" placeholder="{{ __('app.dropoff_placeholder') }}" required>
                            ...
                            <div class="mb-5 p-4 bg-gray-50 rounded-xl space-y-2">
                                ...
                                <div class="border-t border-gray-200 pt-2 flex justify-between items-center">
                                    <span class="text-sm font-semibold text-gray-700">{{ __('app.total_fare') }}</span>
